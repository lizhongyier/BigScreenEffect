...
import { stopDomEventFromClassName } from "@bus/utils/tools";

const withoutEventClassName = "without-TransformControls-event";
const selectedColor = 0xffff00;
const redColor = 0xf92e3b;
const greenColor = 0x00dd67;
const blueColor = 0x008ced;
...
class TransformControls extends Object3D {
  constructor(camera, domElement) {
    ...
    defineProperty("stopUpdate", false);
    defineProperty("axis", null);
    defineProperty("mode", ["translate", "rotate"]);
  ...
  pointerHover(pointer) {
    ...
    const intersectT = intersectObjectWithRay(
      this._gizmo.picker["translate"],
      _raycaster
    );
    const intersectR = intersectObjectWithRay(
      this._gizmo.picker["rotate"],
      _raycaster
    );
    if (intersectT) {
      this.mode = ["translate"];
      this.axis = intersectT.object.name;
    } else if (intersectR) {
      this.mode = ["rotate"];
      this.axis = intersectR.object.name;
    } else {
      this.axis = null;
    }
  }
  ...
  intoUnabledStyle() {
    // this.enabled = false;
    let handles = [];
    let gizmo = this._gizmo.gizmo;
    ["translate", "rotate"].forEach((mode) => {
      handles = handles.concat(gizmo[mode].children);
    });
    handles.forEach((handle) => {
      handle.material.color.setHex(0x999999);
    });
    return this;
  }
...
function onPointerHover(event) {
  // 剔除不能操作的dom
  if (stopDomEventFromClassName(event, withoutEventClassName)) {
    return;
  }
  if (!this.enabled) return;
...
class TransformControlsGizmo extends Object3D {
  constructor() {
    ...
    matRed.color.setHex(redColor);

    const matGreen = gizmoMaterial.clone();
    matGreen.color.setHex(greenColor);

    const matBlue = gizmoMaterial.clone();
    matBlue.color.setHex(blueColor);

    const matRedTransparent = gizmoMaterial.clone();
    matRedTransparent.color.setHex(redColor);
    matRedTransparent.opacity = 0.5;

    const matGreenTransparent = gizmoMaterial.clone();
    matGreenTransparent.color.setHex(greenColor);
    matGreenTransparent.opacity = 0.5;

    const matBlueTransparent = gizmoMaterial.clone();
    matBlueTransparent.color.setHex(blueColor);
    matBlueTransparent.opacity = 0.5;

    const matWhiteTransparent = gizmoMaterial.clone();
    matWhiteTransparent.color.setHex(0xe8a1fa);
    // matWhiteTransparent.opacity = 0.25;

    const matYellowTransparent = gizmoMaterial.clone();
    matYellowTransparent.color.setHex(selectedColor);
    matYellowTransparent.opacity = 0.25;

    const matYellow = gizmoMaterial.clone();
    matYellow.color.setHex(selectedColor);

    const matGray = gizmoMaterial.clone();
    matGray.color.setHex(0xe8a1fa);
    ...
    const lineGeometry2 = new CylinderGeometry(0.0075, 0.0075, 1, 3);
    ...
    const gizmoTranslate = {
      ...
      Y: [
        ...
        [new Mesh(arrowGeometry, matGreen), [0, -1, 0], [Math.PI, 0, 0]],
    ...
    const pickerTranslate = {
      X: [
        [
          new Mesh(new CylinderGeometry(0.05, 0, 1, 4), matInvisible),
          [0.5, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
        [
      ...
      XYZ: [[new Mesh(new OctahedronGeometry(0.05, 0), matInvisible)]],
      XY: [
        [
    ...
    const gizmoRotate = {
      ...
      X: [[new Mesh(CircleGeometry(0.5, 0.5), matRed.clone())]],
      Y: [
        [
          new Mesh(CircleGeometry(0.5, 0.5), matGreen.clone()),
          null,
          [0, 0, -Math.PI / 2],
        ],
      ],
      Z: [
        [
          new Mesh(CircleGeometry(0.5, 0.5), matBlue.clone()),
          null,
          [0, Math.PI / 2, 0],
        ],
      ],
    ...
    const pickerRotate = {
      ...
      X: [
        [
          new Mesh(new TorusGeometry(0.5, 0.03, 4, 24), matInvisible),
          [0, 0, 0],
    ...
    const gizmoScale = {
      X: [
        [
          new Mesh(scaleHandleGeometry, matRed.clone()),
          [0.5, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
        [
          new Mesh(lineGeometry2, matRed.clone()),
          [0, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
        [
      ...
      Y: [
        [new Mesh(scaleHandleGeometry, matGreen.clone()), [0, 0.5, 0]],
        [new Mesh(lineGeometry2, matGreen.clone())],
        [
          new Mesh(scaleHandleGeometry, matGreen.clone()),
          [0, -0.5, 0],
          [0, 0, Math.PI],
        ],
      ],
      Z: [
        [
          new Mesh(scaleHandleGeometry, matBlue.clone()),
          [0, 0, 0.5],
          [Math.PI / 2, 0, 0],
        ],
        [
          new Mesh(lineGeometry2, matBlue.clone()),
          [0, 0, 0],
          [Math.PI / 2, 0, 0],
        ],
        [
    ...
    function setupGizmo(gizmoMap, modeType) {
    ...
    this.add(
      (this.gizmo["translate"] = setupGizmo(gizmoTranslate, "translate"))
    );
    this.add((this.gizmo["rotate"] = setupGizmo(gizmoRotate, "rotate")));
    this.add((this.gizmo["scale"] = setupGizmo(gizmoScale, "scale")));
    this.add(
      (this.picker["translate"] = setupGizmo(pickerTranslate, "translate"))
    );
  ...
  updateMatrixWorld(force) {
    ...
    const space = this.mode.indexOf("scale") !== -1 ? "local" : this.space; // scale always oriented to local rotation
    ...
    const showTF =
      this.mode.indexOf("translate") !== -1 ||
      this.mode.indexOf("rotate") !== -1;

    this.gizmo["translate"].visible = showTF;
    this.gizmo["rotate"].visible = showTF;
    this.gizmo["scale"].visible = this.mode.indexOf("scale") !== -1;

    this.helper["translate"].visible = showTF;
    this.helper["rotate"].visible = showTF;
    this.helper["scale"].visible = this.mode.indexOf("scale") !== -1;
    ...
    for (let i = 0; i < handles.length; i++) {
      ...
      const modeType = handle.modeType;
      ...
      if (handle.tag === "helper") {
        handle.visible = false;
        if (this.mode.indexOf(modeType) !== -1) {
          if (handle.name === "AXIS") {
      ...
      } else if (this.mode.indexOf("rotate") !== -1) {
        // Align handles to current local or world rotation
      ...
      if (this.enabled && this.axis) {
        if (handle.name === this.axis && this.mode.indexOf(modeType) !== -1) {
          handle.material.color.setHex(selectedColor);
          ...
          if (modeType === "translate" && this.mode.indexOf(modeType) !== -1) {
            if (handle.geometry.type !== "TorusGeometry") {
              handle.material.color.setHex(selectedColor);
              handle.material.opacity = 1.0;
            }
          }
...
class TransformControlsPlane extends Mesh {
  ...
  updateMatrixWorld(force) {
    ...
    if (this.mode.indexOf("scale") !== -1) space = "local"; // scale always oriented to local rotation
